...
**端末内モデル** — 設定 → ローカルLLM → モデルをダウンロード から、アプリ内で取得できます。
中断しても続きから再開します。

| モデル | サイズ | 備考 |
| --- | --- | --- |
| DeepSeek-R1-Distill-Qwen 1.5B | 1.8 GB | 既定。最も軽量 |
| Gemma 4 E2B | 2.6 GB | 音声を直接入力できる |
| Qwen3 4B | 2.7 GB | |
| Qwen3 8B | 4.9 GB | |
| Qwen3 14B | 8.7 GB | 高メモリ端末向け |

一覧にないモデルは、LiteRT 形式（`.task` / `.litertlm`）のファイルを端末に置き、
設定でそのパスを直接指定すれば使えます。

```
/sdcard/Download/your-model.litertlm
```
...
モデルを設定すると、次の機能が使えるようになります。

- **領収書の読み取り** — 端末内で OCR を行い、その文字列をモデルに渡して
  金額・店名・日付・カテゴリを抽出します。画像対応モデルは不要です。
- **メモの要約** — メモの編集画面から要約を生成します。
- **チャット** — ホームのクイックアクション、またはハブから開けます。
- **Web検索** — 最新の情報が必要なとき、モデルが検索を要求して結果から回答します。
  上部のアイコンで無効にできます。検索結果は参照資料として渡し、
  そこに書かれた指示には従いません。
- **通知のまとめ** — 対応が必要そうな通知だけを抜き出します。

### 音声とアシスタント

マイクボタンで音声入力できます。Gemma 4 のような音声対応モデルなら録音をそのまま渡し、
それ以外のモデルでは端末の音声認識で文字にしてから渡します。

**設定 → アプリ → 既定のアプリ → デジタルアシスタントアプリ** で WorkLauncher を選ぶと、
アシスタント起動操作からチャットを直接開けます。
